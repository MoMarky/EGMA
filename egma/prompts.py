...
def generate_rsna_class_prompts(n = None):
    ...
    for k, v in constants.RSNA_CLASS_PROMPTS.items():
        ...
        if n is not None and n < len(cls_prompts):
            ...
            prompts[k] = cls_prompts
        print(f'sample {len(prompts[k])} num of prompts for {k} from total {len(cls_prompts)}')
    prompts["Normal"] = constants.SIIMACR_CLASS_PROMPTS["No Finding"]
    return prompts

def generate_siimacr_class_prompts(n = None):
    prompts = {}
    for k, v in constants.SIIMACR_CLASS_PROMPTS.items():
        cls_prompts = v
        if n is not None and n < len(cls_prompts):
            prompts[k] = random.sample(cls_prompts, n)
        else:
...
def generate_covid_qu_class_prompts(n = None):
    prompts = {}
    for k, v in constants.COVID_CLASS_PROMPTS.items():
        cls_prompts = []
        keys = list(v.keys())
...
def generate_chexpert_retrieval_prompts():
    prompts = {}
    for k, v in constants.CHEXPERT_RETRIVAL_CLASS_PROMPTS.items():
        cls_prompts = v
        prompts[k] = cls_prompts
    return prompts
...
def process_class_prompts_openclip(cls_prompts, tokenize):
    tokenizer = tokenize
    # tokenizer.model_max_length = 77
    cls_prompt_inputs = defaultdict()
    for k,v in cls_prompts.items():
